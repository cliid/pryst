...
in_directory = os.path.join(os.getcwd(), 'pryst/tests')
...
pryst_binary = sys.executable + ' ' + \
    os.path.join(os.getcwd(), 'test_entryp.py')
...
class ProperParserTest(FileTestCase):
    ...
    def runTest(self):
        result = subprocess.run(
            pryst_binary + " --parse " + self.file_path, stdout=PIPE, stderr=STDOUT,
...
class SemanticTest(FileTestCase):
    ...
    def runTest(self):
        pryst_result = subprocess.run(
            pryst_binary + ' -q ' + self.file_path, stdout=PIPE, stderr=STDOUT,
            shell=True, universal_newlines=True, env=envs
        )
        ...
        self.assertEqual(pryst_result.returncode, python_result.returncode)
        self.assertEqual(pryst_result.stdout, python_result.stdout)
...
class ShellTest(FileTestCase):
    ...
    def runTest(self):
        ...
        with open(self.file_path) as fstdin:
            pryst_result = subprocess.run(
                pryst_binary + ' -q -i ', stdin=fstdin, stdout=PIPE, stderr=STDOUT,
                shell=True,  env=envs,
            )
...
class EvalTest(unittest.TestCase):
    ...
    def runTest(self):
        from pryst.prystapp import pryst_eval
        expected = eval(self.expression)
        actual = pryst_eval(self.expression)
