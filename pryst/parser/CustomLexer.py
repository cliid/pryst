...
from pryst.parser.Errors import IndentationErr
from pryst.parser.PrystLexer import PrystLexer
...
class CustomLexer(PrystLexer):
    ...
    def nextToken(self):
        ...
        if self._input.LA(1) == Token.EOF and len(self.indents) != 0:
            ...
            i = len(self.tokens) - 1
            while i >= 0:
                if self.tokens[i].type == PrystParser.EOF:
                    del self.tokens[i]
                i -= 1

            # First emit an extra line break that serves as the end of the statement
            self.emitToken(self.commonToken(PrystParser.NEWLINE, '\n'))
            ...
            self.emitToken(self.commonToken(PrystParser.EOF, "<EOF>"))
    ...
    def createDedent(self):
        dedent = self.commonToken(PrystParser.DEDENT, "")
    ...
    def newLineAction(self):
        ...
        if indent == previous:
            ...
            self.emitToken(self.commonToken(PrystParser.INDENT, spaces))
        else:
