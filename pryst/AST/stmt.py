...
from pryst.AST.ast import Statement, Expression, MemoryContext
from pryst.AST.expr import AddOp, SubOp, MultOp, DivOp, ModOp, LshiftOp, RshiftOp, BinOp, UnaryOp, Compare
from pryst.AST.expr import BitAndOp, BitOrOp, BitXorOp, Name, CallExpr
...
class FunctionDef(Statement):
    ...
    def eval(self) -> None:
        ...
        def container(*args):
            namespace = runtime.Memory.Namespace(
                outerScope=declarationNamespace)
    ...
    def __init__(self, target, iter, body, orelse=None):
        ...
        if orelse is not None:
            raise NotImplementedError(
                "You should implement orelse in grammar first!")
...
class Attribute(Statement):
    ...
    def eval(self):
        ...
        if self.ctx == MemoryContext.Load:
            ...
            raise NotImplementedError(
                "Assigning to attributes is not supported!")
            #
        ...
        try:
            if isinstance(self.slice, Index):
                ...
                if self.ctx == MemoryContext.Load:
                    ...
                    raise NotImplementedError(
                        "Writing to slices & deleting elements is not supported")
