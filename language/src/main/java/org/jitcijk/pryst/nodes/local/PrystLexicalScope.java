 ...
 */
public final class PrystLexicalScope {

    private final Node current;
    private final PrystBlockNode block;
    private final PrystBlockNode parentBlock;
    private final RootNode root;
    private PrystLexicalScope parent;
     ...
     */
    private PrystLexicalScope(Node current, PrystBlockNode block, PrystBlockNode parentBlock) {
     ...
     */
    private PrystLexicalScope(Node current, PrystBlockNode block, RootNode root) {
        this.current = current;
        this.block = block;
    ...
    public static PrystLexicalScope createScope(Node node) {
        PrystBlockNode block = getParentBlock(node);
        if (block == null) {
            // We're in the root.
        ...
        PrystBlockNode parentBlock = getParentBlock(block);
        if (parentBlock == null) {
            return new PrystLexicalScope(node, block, block.getRootNode());
        } else {
            return new PrystLexicalScope(node, block, parentBlock);
        }
    }

    private static PrystBlockNode getParentBlock(Node node) {
        PrystBlockNode block;
        Node parent = node.getParent();
        // Find a nearest block node.
        while (parent != null && !(parent instanceof PrystBlockNode)) {
            parent = parent.getParent();
        }
        if (parent != null) {
            block = (PrystBlockNode) parent;
        } else {
            block = null;
        }
        return block;
    ...
    private static PrystBlockNode findChildrenBlock(Node node) {
        PrystBlockNode[] blockPtr = new PrystBlockNode[1];
        node.accept(new NodeVisitor() {
            @Override
            public boolean visit(Node n) {
                if (n instanceof PrystBlockNode) {
                    blockPtr[0] = (PrystBlockNode) n;
                    return false;
                } else {
                    return true;
                }
    ...
    public PrystLexicalScope findParent() {
        if (parentBlock == null) {
        ...
        if (parent == null) {
            Node node = block;
            PrystBlockNode newBlock = parentBlock;
            // Test if there is a next parent block. If not, we're in the root scope.
            PrystBlockNode newParentBlock = getParentBlock(newBlock);
            if (newParentBlock == null) {
                parent = new PrystLexicalScope(node, newBlock, newBlock.getRootNode());
            } else {
                parent = new PrystLexicalScope(node, newBlock, newParentBlock);
            }
        }
        return parent;
    }
    ...
    private Map<String, FrameSlot> collectVars(Node varsBlock, Node currentNode) {
        ...
        NodeUtil.forEachChild(varsBlock, new NodeVisitor() {
            ...
            public boolean visit(Node node) {
                ...
                if (!(node instanceof PrystBlockNode)) {
                ...
                if (node instanceof PrystWriteLocalVariableNode) {
                    PrystWriteLocalVariableNode wn = (PrystWriteLocalVariableNode) node;
    ...
    private static Map<String, FrameSlot> collectArgs(Node block) {
        ...
        NodeUtil.forEachChild(block, new NodeVisitor() {
            ...
            private PrystWriteLocalVariableNode wn; // The current write node containing a slot
            ...
            public boolean visit(Node node) {
                ...
                if (node instanceof PrystWriteLocalVariableNode) {
                    ...
                    wn = null;
                    return all;
                } else if (wn != null && (node instanceof PrystReadArgumentNode)) {
