...
public class PrystJavaInteropConversionTest {
    ...
    public void testGR7318Object() throws Exception {
        String sourceText = "function test(validator) {\n" +
                        ...
                        "}";
        try (Context context = Context.newBuilder(PrystLanguage.ID).build()) {
            context.eval(Source.newBuilder(PrystLanguage.ID, sourceText, "Test").build());
            Value test = context.getBindings(PrystLanguage.ID).getMember("test");
    ...
    public void testGR7318List() throws Exception {
        String sourceText = "function test(validator, array) {\n" +
                        ...
                        "}";
        try (Context context = Context.newBuilder(PrystLanguage.ID).allowHostAccess(HostAccess.ALL).build()) {
            context.eval(Source.newBuilder(PrystLanguage.ID, sourceText, "Test").build());
            Value test = context.getBindings(PrystLanguage.ID).getMember("test");
